...
export default {
    setup() {
        ...
        const registerCustomer = async () => {
            try {
                const response = await api.post('/user', { username: username.value, password: password.value, role_name: 'customer' });

                const user_id = response.data.user_id;

                const registerCust = await api.post(`/customer`, {
                    user_id: user_id,
                    email: email.value,
                    name: name.value,
